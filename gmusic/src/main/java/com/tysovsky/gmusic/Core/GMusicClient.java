...
public class GMusicClient {
    ...
    public String getStreamingUrl(GMusicSong song){
        ...
        try (Response response = httpClient.newCall(RequestManager.streamUrlRequest(song)).execute()) {
    ...
    public void getStreamingUrlAsync(final GMusicSong song, final GetStreamUrlListener listener){
        httpClient.newCall(RequestManager.streamUrlRequest(song)).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                listener.OnCompleted(Status.FAILURE, null);
            }
